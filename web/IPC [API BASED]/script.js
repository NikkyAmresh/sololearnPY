    ...
    $.ajax({
        ...
        success: function(data) {
            switch (n) {
                ...
                case 2:
                    for (var i = 0; i < data['data']['result'].length; i++) {
                        $("#fill-sec").append("<li><a onclick=section('" + data['data']['result'][i]['number'] + "') href='#'> Section " + data['data']['result'][i]['number'] + "</a>-"+data['data']['result'][i]['title']+ "<small> <small><a href='#' onclick=chapter('" + data['data']['result'][i]['chapter'] + "')>  (Chapter " + data['data']['result'][i]['chapter'] + " ) </a></small></small></li><hr>")
                    }
                    break;
                case 3:
                    $("#sec").append("<h5 class=text-center><a onclick=chapter('" + data['data']['result'][0]['chapter'] + "') href='#'>Chapter  " + data['data']['result'][0]['chapter'] + "</a><small> (Section " + data['data']['result'][0]['from'] + " )</small></h5>" + data['data']['result'][0]['chap-title'] + "<hr><h3>Section " + data['data']['number'] + "</h3><h6>"+data['data']['result'][0]['title']+"</h6><hr><p class=text-justify> " + data['data']['result'][0]['text'] + " </p>")
                    break;
                case 4:
                    $("#chap").html("<h3 class=text-center>Chapter " + data['data']['number'] + "<small> (Section" + data['data']['from'] + ")</small></h3>" + data['data']['description'] + "<p><hr>")
                    for (var i = 0; i < data['data']['result'].length; i++) {
                        $("#chap").append("<h5 class=text-justify><a onclick=section('" + data['data']['result'][i]['number'] + "') href=#>Section" + data['data']['result'][i]['number'] + "</a></h5><p class=text-justify>" + data['data']['result'][i]['title']+ "</p><hr>")
                    }
                    break;
                case 5:
