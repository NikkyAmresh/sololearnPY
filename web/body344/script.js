...
var name = prompt("Thanks for a 100 upvotes!\n\nNow you can submit your score to the leaderboard(button in top right corner)\n\nWrite your name in this prompt to submit your score to the leaderboard or hit cancel to not submit\n\nleaderboard resets everyday! (DON'T CHEAT!)","nulll");
...
var start = function () {
    ...
    if(!device) window.addEventListener("mousedown", mousePress)
    window.addEventListener("touchmove", touchMove);
    if(!device) window.addEventListener("mousemove", mouseMove);
    window.addEventListener("touchend", touchEnd);
    
    listElt = document.querySelector("#leaderboard");
...
var loop = function () {
    ...
    if (gamemode == 3) {
        ...
        ctx.fillText("SoloLearn Edition", width/2, 90);
        ctx.font = "20px Arial";
        ctx.fillText("Made by Ask Plays", width/2, 120);
    ...
    for(var i=0; i<obstacles.length; i++) {
        ...
        if(obstacles[i].y+scroll>height+30) {
            ...
            gamemode = 2;
            score = 677543575674446;
            scroll = 677543575456446;
            speed = 2.5;
...
function mousePress(e) {
    ...
    if (mx > width/2-65 && mx < width/2+65 && my>430 && my<465 && (gamemode == 0 || gamemode == 2)) {
        skin = (skin+1)%colors.length;
