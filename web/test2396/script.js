...
$(function(){firebase.database().ref('songs/').on("child_added", function(snap) {
  var songss = snap.val();
  var songnamed=songss.song_name;
  var pic=songss.picurl;    
 apnd = $("#container").append("<div id='added' class='pl'><img src=" + pic + " height='40px'width='50px'>" + songnamed + "<div class='dropdown' style='float:right;'><img class='dropbtn' src = 'http://www.vvillageresorts.com/tour/skin/Image_2FB6632C_20CB_DBE7_41AE_0C4F8CFBCC75.png' height='40px'> <div class='dropdown-content'><a href=#>Download</a><a href=# class='like'>Like</a>  <a href=# class = 'share'>Share</a>  <a href=# class = 'del'>Delete</a> </div></div></div>");});});
function addsong(){
...
if((imslice !== ("jpg"||"png")) && (sslice !== ("mp3" || "ogg"))){
 ...
 add(song_name,song_url,img_url)
/* firebase.database().ref('songs/').push(song_url);
firebase.database().ref('images/').push(img_url);
firebase.database().ref('songName/').push(song_name);
 songs.push(song_url); 
 images.push(img_url); 
     */
//-------------once------------------------\\
